...
test.describe('Navigation', () => {
  ...
  test('should redirect to tools page', async ({ page }) => {
    ...
    await expect(page).toHaveURL('http://localhost:5173/tools');
  });
});
...
test.describe('Home page projects', () => {
  projects.forEach(project => {
    test(`should contain ${project.title} project`, async ({ page }) => {
      const projectCard = page.locator(`#${project.route}`);
      // const projectTitle = page.getByText(project.title);
  ...
  projects.forEach(project => {
    test(`should redirect to ${project.title} project page`, async ({ page }) => {
      page.locator(`#${project.route}`).click();
      
      await expect(page).toHaveURL(`http://localhost:5173/projects/${project.route}`);
    });
  });
});
